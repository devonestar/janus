...
function parseTargetedDoomReport(raw: string): TargetedDoomReport | null {
  const attempt = (text: string): TargetedDoomReport | null => {
    try {
      ...
      if (
        ...
        Array.isArray((parsed as Record<string, unknown>)["doom_scenarios"])
      ) {
        return parsed as TargetedDoomReport;
      }
    } catch {
      // fall through
    }
  ...
  try {
    ...
    if (
      ...
      const inner = (envelope as Record<string, unknown>)["result"];
...
function buildCrosscheckMatrix(
  conditions: string[],
  scenarios: TargetedDoomScenario[],
): CrosscheckEntry[] {
  return conditions.map((condition, i) => {
    ...
    let risk_level: HarnessRiskLevel;
    if (!covering) {
      risk_level = "uncovered";
    } else {
      const sev = covering.severity as string;
      if (sev === "fatal" || sev === "severe" || sev === "moderate" || sev === "low") {
        risk_level = sev as HarnessRiskLevel;
      } else {
        risk_level = "uncovered";
      }
    }
...
function computeHarnessVerdict(
  evalOutput: JanusOutput,
...
export async function runHarness(
  ...
  if (evalResponse.error || !evalResponse.parsed) {
    throw new Error(
      `Pass 1 (eval) failed: ${evalResponse.error ?? "no parsed output"}`,
    );
  }
  ...
  const conditions: string[] = evalOutput.best_path?.enabling_conditions ?? [];

  if (conditions.length === 0) {
    process.stderr.write("[harness] No enabling conditions found — skipping targeted doom.\n");
    const emptyDoom: TargetedDoomReport = {
      doom_scenarios: [],
      survival_rating: "antifragile",
      doom_count: 0,
    };
    const emptyCrosscheck: CrosscheckEntry[] = [];
  ...
  if (!doomReport) {
    throw new Error(
      `Pass 2 (targeted doom) failed: could not parse response. Raw:\n${doomResponse.raw.slice(0, 500)}`,
    );
  }
  ...
  process.stderr.write(
    `[harness] Done. survival_rate=${(verdict.condition_survival_rate * 100).toFixed(0)}%, ` +
...
export function formatHarnessOutput(report: HarnessReport, format: "json" | "markdown"): string {
  if (format === "json") {
