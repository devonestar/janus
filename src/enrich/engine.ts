...
import type { EnrichmentReport, EnrichmentFinding } from "../types.js";
...
function parseFindings(raw: string): EnrichmentFinding[] {
  const attempt = (text: string): EnrichmentFinding[] | null => {
    try {
      ...
      if (
        ...
        return (parsed as { findings: EnrichmentFinding[]; summary?: string }).findings;
      }
    } catch {
      // fall through
    }
...
function parseSummary(raw: string): string {
  ...
  if (match) {
    try {
      const p: unknown = JSON.parse(match[0]);
      if (typeof p === "object" && p !== null && "summary" in p) {
        const s = (p as Record<string, unknown>)["summary"];
        if (typeof s === "string") return s;
      }
    } catch {
      // fall through
...
export function formatEnrichOutput(report: EnrichmentReport, format: "json" | "markdown"): string {
  if (format === "json") {
  ...
  if (report.findings.length === 0) {
    ...
    for (const f of report.findings) {
      ...
      if (f.supports_assumption) lines.push(`**Supports**: ${f.supports_assumption}`);
      if (f.contradicts_assumption) lines.push(`**Contradicts**: ${f.contradicts_assumption}`);
