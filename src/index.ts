...
function parseDoomResponse(raw: string): DoomReport | null {
  try {
...
async function runEval(
  ...
  model?: string,
  samples = 1,
): Promise<{ output: JanusOutput; exitCode: number } | never> {
  ...
  const finalizeEvalOutput = (output: JanusOutput): JanusOutput => {
...
program
  ...
  .action(async (input: string, opts: { format?: string; backend: string; model?: string }) => {
    try {
      ...
      const available = await backend.isAvailable();
      if (!available) {
        const hints = BACKEND_INSTALL_HINTS[opts.backend];
...
program
  ...
  .action(async (file: string, opts: { format?: string; backend: string; model?: string; samples?: string }) => {
    try {
      const format = resolveFormat(opts.format);
      ...
      const { output, exitCode } = await runEval(
        file,
        opts.backend as BackendType,
        format,
        opts.model,
        samples,
      );
...
program
  ...
  .action(async (fileA: string, fileB: string, opts: { format?: string; backend: string; model?: string }) => {
    try {
      const format = resolveFormat(opts.format);
...
program
  ...
  .option("-b, --backend <backend>", "AI backend", "claude")
  .option("-m, --model <model>", "Model override")
  .action(async (file: string, opts: { backend: string; model?: string }) => {
    try {
      const { output, exitCode } = await runEval(
        file,
        opts.backend as BackendType,
...
const BACKEND_INSTALL_HINTS: Record<string, string[]> = {
...
async function runDoctor(): Promise<void> {
  ...
  const backends: BackendType[] = ["claude", "codex", "opencode", "openai-api", "anthropic-api"];
  ...
  for (const type of backends) {
    ...
    if (!ok) {
      ...
      if (hints) process.stdout.write(`  — ${hints[0]}`);
    }
    process.stdout.write("\n");
