...
export function validateOutput(output: JanusOutput): string[] {
  const errors: string[] = [];
  ...
  for (const rp of output.rejected_paths) {
    ...
    if (rp.comparison_basis) {
      ...
      if (!FRAGILITY.has(cb.fragility)) errors.push(`Rejected path "${rp.name}" comparison_basis.fragility invalid`);
      if (!UNKNOWNS.has(cb.unknowns)) errors.push(`Rejected path "${rp.name}" comparison_basis.unknowns invalid`);
      if (!NFR.has(cb.nfr_coverage)) errors.push(`Rejected path "${rp.name}" comparison_basis.nfr_coverage invalid`);
      if (!REVERSIBILITY.has(cb.reversibility)) errors.push(`Rejected path "${rp.name}" comparison_basis.reversibility invalid`);
      if (!SCOPE.has(cb.scope)) errors.push(`Rejected path "${rp.name}" comparison_basis.scope invalid`);
    }
  }
...
export function formatOutput(output: JanusOutput, format: OutputFormat): string {
  switch (format) {
    case "json":
...
export function validateDoomOutput(output: DoomReport): string[] {
  const errors: string[] = [];
...
export function formatDoomOutput(output: DoomReport, format: OutputFormat): string {
  switch (format) {
    case "json":
      return JSON.stringify(output, null, 2);
    case "yaml":
...
export function formatDoomMarkdown(o: DoomReport): string {
  const lines: string[] = [];
...
function formatMarkdown(o: JanusOutput): string {
  const lines: string[] = [];
