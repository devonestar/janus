...
function deriveArchetypeSlug(path: RejectedPath): string {
  ...
  if (explicit && !weakExplicit) return explicit;
...
export function deriveCanonicalKey(path: RejectedPath): string {
...
export function normalizeRejectedPath(path: RejectedPath): RejectedPath {
  const archetypeSlug = deriveArchetypeSlug(path);
  return {
    ...path,
...
export function normalizeOutputRejectedPaths(output: JanusOutput): JanusOutput {
  return {
    ...output,
