...
export async function runLoop(
  ...
  let prevRejected: number | null = null;
  let prevUnknowns: number | null = null;
  ...
  for (let iter = 1; iter <= maxIter; iter++) {
    ...
    let trend: "improving" | "stagnant" | "degrading" | null = null;
    ...
    if (
      evaluation.decision_status === "recommend" &&
    ...
    if (
      evaluation.decision_status === "conditional" &&
...
function stateToHistory(s: LoopState) {
  return {
