...
import type { HarnessReport, EvaluationRequest } from "../types.js";
...
async function safeResolve(inputPath: string, outputPath: string): Promise<{ safe: boolean; error?: string }> {
...
function validateRefinerOutput(text: string): boolean {
...
function applyPatch(document: string, patch: string): string {
  ...
  for (const section of patchSections) {
    ...
    if (!headingLine || !headingRe.test(headingLine)) continue;
...
export async function runHarnessLoop(
  ...
  const convergenceHistory: import("./convergence.js").ConvergenceMetric[] = [];
  ...
  for (let iter = 1; iter <= maxIterations; iter++) {
    ...
    const overrideReason: LoopTerminationReason | undefined = isBlocked && score > 0 && convergenceHistory.length > 0 && convergenceHistory[convergenceHistory.length - 1].score <= score
      ? "blocked"
      : undefined;
    ...
    if (fatalConditions.length === 0 && unattackedConditions.length === 0) continue;

    const refinerPrompt = buildRefinerPrompt(currentDocument, fatalConditions, unattackedConditions);
    const refinerRequest: EvaluationRequest = {
