export function checkConvergence(history, maxIterations, stagnantThreshold = 2, nonConvergenceWindow = 3) {
    const n = history.length;
    if (n === 0)
        return { terminate: false, reason: null, best_iteration_index: 0 };
    ...
    if (n >= nonConvergenceWindow) {
        ...
        const noReduction = window.every((m, i) => i === 0 || m.score >= window[i - 1].score);
        if (noReduction) {
            return {
                terminate: true,
